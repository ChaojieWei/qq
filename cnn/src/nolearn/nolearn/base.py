...
from nolearn._compat import chain_exception
from nolearn._compat import pickle
        ...
        while epoch < self.max_epochs:
            ...
            for Xb, yb in self.batch_iterator_test(X_valid, y_valid):
                ...
                if self.custom_scores:
                    ...
                    custom_scores.append(self.custom_scores[1](yb, y_prob))

            avg_train_loss = np.mean(train_losses)
            avg_valid_loss = np.mean(valid_losses)
            avg_valid_accuracy = np.mean(valid_accuracies)
            if custom_scores:
                avg_custom_scores = np.mean(custom_scores)
            ...
            if self.custom_scores:
                info[self.custom_scoress[0]] = avg_custom_scoress
